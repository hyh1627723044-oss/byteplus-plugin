...
新增独立安卓应用：[小刷 · 本地关键词遥控器](android/README.md)。它使用 sherpa-onnx 中文关键词检测，
无需 Operit、百度 SDK、LLM 或服务端，口令命中后直接执行手势。源码与 APK 构建说明位于 `android/`。
下文仍是原 Operit 插件的使用说明，二者是独立入口。
